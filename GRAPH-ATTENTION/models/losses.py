...
class NTXent(nn.Module):
    ...
    def forward(self, z,z_2,cur_iter,get_map=False):
        n = z.shape[0]
        ...
        weight_sum=n
        if cur_iter>=0:
            logprob = torch.mul(logprob,sims.detach().cuda())
            weight_sum = sims[np.repeat(np.arange(n), m-1), labels].sum().item()
           
        # TODO: maybe different terms for each process should only be computed here...
        loss = -logprob[np.repeat(np.arange(n), m-1), labels].sum() / weight_sum / (m-1) / self.norm
