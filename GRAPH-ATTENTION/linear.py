...
def add_learner_params(parser):
    ...
    parser.add_argument('--d_ratio', default=-1, type=float, help='clip delete ratio')
...
def main_worker(gpu, ngpus, args):
    ...
    start_epoch, cur_iter, best_acc = hfai.checkpoint.init(model.model, optimizer=optimizer,scheduler=scheduler, ckpt_path=args.resume_path)
    ...
    while continue_training:
        ...
        for _, batch in enumerate(train_loader):
            ...
            if not args.eval_only:
                ...
                logs = model.train_step(batch, cur_iter,args.d_ratio)
                loss = logs['loss']
                ...
                loss.backward()
                optimizer.step()
            model.model.try_save(3,cur_iter+1)   
